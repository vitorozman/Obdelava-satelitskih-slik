...
###############################################################################
# Funkcija za krcene za pozidano
###############################################################################
...
apply_reduction = function(input_array) {
  ...
  tibble_array %>%
    ...
    as.matrix() %>%
    array_reshape(c(nrow(input_array), 3 * TIME))
}
...
ind_80 <- createDataPartition(podatki$ref, p=0.80, list=FALSE)
podatki_80 <- podatki[ind_80,]
...
# podvorcenje
ucnaDown <- downSample(podatki_80[, -37], podatki_80$ref, yname="ref")
# Natreniramo model 

#summary(ucnaDown)
dataX <- ucnaDown[,-37] #3 * 12 + 1 = 37
dataY <- ucnaDown[,37]
...
shrani_podatke_pozidano <- function(model, typ){
...
shrani_podatke_pozidano(RFModel, 8)
